# $Id: DBIStag.pm,v 1.1 2003/04/23 00:59:35 cmungall Exp $
# -------------------------------------------------------
#
...
DBIStag will guess a structure/schema based on the ordering of the
relations in your FROM clause. However, this guess can be overwritten
at either the SQL level (using DBIStag specific SQL extensions) or at
the API level.
...
table a, DBIStag will not detect this. You have to guide it. You can
...
structure using a DBIStag specific SQL extension. The DBIStag specific
...
If you are using the DBIStag API directly, you can pass in the nesting
...
DBIStag returns L<Data::Stag> structures that are equivalent to a
...
=head2 connect
  ...
  Usage   - $dbh = DBIx::DBIStag->connect($DSN);
  Returns - L<DBIx::DBIStag>
  Args    - see L<DBI>
